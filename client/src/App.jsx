...
const seed = {
  ...
  posts: [
    ...
    { id: 'r3', name: 'Simba', birthday: '2024-03-24', tone: 'orange', contact: '099XXXX', description: 'If you give me some time and patience, my shyness will turn into cuddles soon.', photo: '' },
  ],
}
...
function asset(name) { return `${import.meta.env.BASE_URL}assets/${name}` }
function fallbackImage(event) { event.currentTarget.onerror = null; event.currentTarget.src = asset('paw_pet.png') }
function emojiFor(species = '', tone = '') { if (tone === 'yellow') return '🐱'; if (tone === 'orange') return '🐶'; if (species === 'Cats') return '🐱'; if (species === 'Dogs') return '🐶'; return '🐾' }
function Photo({ src, species, tone, className }) { const [failed, setFailed] = useState(false); return src && !failed ? <img className={className} src={src} onError={() => setFailed(true)} alt="" /> : <div className={`${className} photo-placeholder`} role="img" aria-label="Pet placeholder">{emojiFor(species, tone)}</div> }
function UploadBox({ value, onChange }) { return <label className={`upload-box ${value ? 'has-file' : ''}`}><input type="file" accept="image/*" onChange={onChange} /><span className="upload-icon">📷</span><strong>{value ? 'Photo selected' : 'Upload a photo'}</strong><small>{value ? 'Click to replace it' : 'Tap here to choose an image'}</small></label> }
const BASE_PATH = import.meta.env.BASE_URL.replace(/\/$/, '')
function route(path) { return `${BASE_PATH}${path === '/' ? '/' : path}` }
...
function PetCard({ pet, index, openPet, onEdit, onDelete }) {
  return <article className={`pet t${index % 3}`}><button type="button" className="pet-open" onClick={() => openPet(pet.id)}><Photo className="pet-img" src={pet.photo} species={pet.species} /><div className="pet-body"><h2>{pet.name}</h2><p className="breed">{pet.breed}</p><p className="meta">Born {formatDate(pet.birthday)}</p></div></button><div className="pet-actions"><button type="button" className="mini-action" onClick={(event) => { event.stopPropagation(); onEdit(pet) }}>Edit</button><button type="button" className="mini-action danger-text" onClick={(event) => { event.stopPropagation(); onDelete(pet.id) }}>Delete</button></div></article>
}
...
function Rehoming({ data, setData, user, onLogin }) {
  const [form, setForm] = useState({ petId: '', description: '', contact: '' }); const [shareId, setShareId] = useState('')
  const url = (post) => `${window.location.origin}${import.meta.env.BASE_URL}rehoming?post=${post.id}`
  ...
  async function share(post) { await navigator.clipboard.writeText(url(post)); setShareId(post.id) }
  return <><div className="page-head"><div><h1 className="page-title">Rehoming</h1><p className="lede">Help a pet find a new family.</p></div><button className="btn yellow" onClick={() => navigator.clipboard.writeText(`${window.location.origin}${import.meta.env.BASE_URL}rehoming`)}>Share this page</button></div><div className="notice"><span>◉</span><p>Contact details in a post are visible to everyone.</p></div>{user ? <form className="card" onSubmit={submit}><h2>Post a pet for rehoming</h2><div className="form-row"><label className="field"><span>Which pet will you rehome?</span><select value={form.petId} onChange={(event) => setForm({ ...form, petId: event.target.value })} required><option value="">Choose a pet</option>{data.pets.map((pet) => <option key={pet.id} value={pet.id}>{pet.name}</option>)}</select></label><label className="field"><span>Description</span><textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} required /></label><label className="field"><span>Contact</span><input value={form.contact} onChange={(event) => setForm({ ...form, contact: event.target.value })} required /><small>Shown publicly on the post.</small></label></div><div className="form-foot"><button className="btn primary">Publish post</button></div></form> : <div className="card cta"><div><h2>Have a pet who needs a new home?</h2><p>Sign in with Google to post them here.</p></div><button className="btn primary" onClick={onLogin}>Sign in with Google</button></div>}<div className="grid posts">{data.posts.map((post) => <article className="post" key={post.id}><span className="ribbon">Looking for a home</span><img className="post-img" src={post.photo || asset('paw_pet.png')} onError={fallbackImage} alt="" /><div className="post-body"><h3>Hello, I'm {post.name}</h3><p className="small">Born {formatDate(post.birthday)}</p><p className="contact">Contact: {post.contact}</p><p className="desc">{post.description}</p><button className="btn outline share" onClick={() => share(post)}>Share link</button>{shareId === post.id && <small>Link copied.</small>}</div></article>)}</div></>
}

function FunctionalDiary({ data, setData, pet, tab, setTab, goRehome, editPet, deletePet }) {
...
function FunctionalRehoming({ data, setData, user, onLogin }) {
  const [form, setForm] = useState({ petId: '', description: '', contact: '' }); const [editing, setEditing] = useState(null)
  function submit(event) { event.preventDefault(); const pet = data.pets.find((item) => item.id === form.petId); if (!pet) return; const post = { ...pet, ...form, id: editing || crypto.randomUUID(), ownerUid: user.uid }; setData({ ...data, posts: editing ? data.posts.map((item) => item.id === editing ? post : item) : [post, ...data.posts] }); setForm({ petId: '', description: '', contact: '' }); setEditing(null) }
  function remove(id) { if (window.confirm('Delete this rehoming post?')) setData({ ...data, posts: data.posts.filter((post) => post.id !== id) }) }
  function edit(post) { setEditing(post.id); setForm({ petId: post.petId || '', description: post.description, contact: post.contact }) }
  return <><div className="page-head"><div><h1 className="page-title">Rehoming</h1><p className="lede">Help a pet find a new family.</p></div><button className="btn yellow" onClick={() => navigator.clipboard.writeText(`${window.location.origin}${import.meta.env.BASE_URL}rehoming`)}>Share this page</button></div><div className="notice"><span>◉</span><p>Contact details in a post are visible to everyone.</p></div>{user ? <form className="card" onSubmit={submit}><h2>{editing ? 'Edit rehoming post' : 'Post a pet for rehoming'}</h2><div className="form-row"><label className="field"><span>Which pet will you rehome?</span><select value={form.petId} onChange={(event) => setForm({ ...form, petId: event.target.value })} required><option value="">Choose a pet</option>{data.pets.map((pet) => <option key={pet.id} value={pet.id}>{pet.name}</option>)}</select></label><label className="field"><span>Description</span><textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} required /></label><label className="field"><span>Contact</span><input value={form.contact} onChange={(event) => setForm({ ...form, contact: event.target.value })} required /></label></div><div className="form-foot"><button className="btn primary">{editing ? 'Save changes' : 'Publish post'}</button></div></form> : <div className="card cta"><div><h2>Have a pet who needs a new home?</h2><p>Sign in with Google to post them here.</p></div><button className="btn primary" onClick={onLogin}>Sign in with Google</button></div>}<div className="grid posts">{data.posts.map((post) => <article className="post" key={post.id}><span className="ribbon">Looking for a home</span><Photo className="post-img" src={post.photo} tone={post.tone} /><div className="post-body"><h3>Hello, I'm {post.name}</h3><p className="small">Born {formatDate(post.birthday)}</p><p className="contact">Contact: {post.contact}</p><p className="desc">{post.description}</p><div className="row"><button className="btn outline share" onClick={() => navigator.clipboard.writeText(`${window.location.origin}${import.meta.env.BASE_URL}rehoming?post=${post.id}`)}>Share link</button>{user && post.ownerUid === user.uid && <><button className="mini-action" onClick={() => edit(post)}>Edit</button><button className="mini-action danger-text" onClick={() => remove(post.id)}>Delete</button></>}</div></div></article>)}</div></>
}

function SharePanel() {
  const link = `${window.location.origin}${import.meta.env.BASE_URL}rehoming`
  const copy = () => navigator.clipboard.writeText(link)
  const facebook = () => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(link)}`, '_blank', 'noopener,noreferrer')
  const instagram = async () => { if (navigator.share) return navigator.share({ title: 'tailTALES rehoming pets', text: 'Pets looking for a new home', url: link }); await navigator.clipboard.writeText(`Pets looking for a new home\n${link}`) }
...
function LegacyEnhancedRehoming({ data, setData, user, onLogin }) {
  ...
  async function copy(value, text) { try { await navigator.clipboard.writeText(value); setMessage(text) } catch { window.prompt('Copy this link', value) } }
  function facebook(value) { window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(value)}`, '_blank', 'noopener,noreferrer') }
  async function instagram(value) { if (navigator.share) { try { await navigator.share({ title: 'tailTALES rehoming', text: 'Pets looking for a new home', url: value }); setMessage('Share menu opened.') } catch { setMessage('Share cancelled.') } } else await copy(`Pets looking for a new home\n${value}`, 'Caption and link copied for Instagram.') }
  function submit(event) { event.preventDefault(); const pet = data.pets.find((item) => item.id === form.petId); if (!pet) return; setData({ ...data, posts: [{ ...pet, ...form, id: crypto.randomUUID(), ownerUid: user.uid }, ...data.posts] }); setForm({ petId: '', description: '', contact: '' }) }
